(function() {
    ...
    var utils = UM.utils,
        ...
        Base = {
            ...
            callback: function(editor, $w, url, state, data) {
                ...
                if (data.success) {
                    state = "SUCCESS";
                    url = editor.options.imagePath + data.result[0];
                } else {
                    url = editor.options.imagePath + url;
                }
                if (state == "SUCCESS") {
                    //显示图片计数+1
                    Upload.showCount++;
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<form class="edui-image-form" method="post" enctype="multipart/form-data">' +
        ...
        config: function(sel) {
            ...
            $.each(me.editor.options.imageFormData, function(key, value) {
                $("form", $(sel, me.dialog)).append("<input type='hidden' name='" + key + "' value='" + value + "'/>");
            });
        ...
        uploadComplete: function(r) {
            ...
            try {
                ...
                Base.callback(me.editor, me.dialog, '', (lang && lang.uploadError) || 'Error!', json);
            }
        },
                ...
                me.dialog.find('.edui-image-content').on('drop', function(e) {
                    ...
                    $.each(fileList, function(i, f) {
                        if (/^image/.test(f.type)) {
                            ...
                            var imageFormData = me.editor.getOpt('imageFormData');
                            $.each(imageFormData, function(key, value) {
                                fd.append(key, value);
                            });
                            xhr.send(fd);
                            xhr.addEventListener('load', function(e) {
